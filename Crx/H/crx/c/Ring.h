...
--------9-----------
--------------------
RINGS INHERENTLY CAN ONLY USE UP TO HALF THE MAXIMUM ADDRESSABLE MEMORY. IF THE MAX IS 2^32, RINGS
CAN ONLY USE 2^31. THIS IS BECAUSE OF THE NEED TO ADD THE INDEX TO THE STARTING ADDRESS. REMEMBER, 
HOWEVER, WITH THE WRONG DESIGN THIS CAN BE REDUCED TO 1/3 THE MAXIMUM ADDRESSABLE MEMORY. THIS IS A 
REMINDER OF ONE OF THE ORIGINAL PROBLEM FACED WHEN DESIGNING THE ALGORITHMS. REMEMBER THAT THIS 
PROBLEM EXISTS BECAUSE OF THE NEED OF AVOIDING OVERFLOW.

I LATER INTRODUCED A MORE THOUROUGH ALGORITHM FOR THE MACRO CRX__C__Ring__ECHO_INSERT_SPACE_AT() 
WHICH TAKES INTO CONSIDERATION THE CLOSEST PATH TO THE EDGE TO AVOID MOVING AS MANY ELEMENTS. THE 
ABOVE IS A REMINDER, A HINT, AS TO WHY THE ALGORITHM APPEARS MORE VERBOSE THAN NECESSARY. IF 
OVERFLOW IS NOT CONSIDERED, THEN IN THE SITUATION OF ROTATING LEFT, MEANING WHEN MOVING BACK THE 
STARTING INDEX, MEANING WHEN ONE HAS TO SUBTRACT pWIDTH FROM THE STARTING INDEX, ONE COULD ADD
THE CAPACITY FIRST, AND THEN SUBTRACT. IF I REMEMBER CORRECTLY, THIS IS THE REASON WHY I DID NOT 
IMPLEMENT THE ALGORITHM THIS WAY THE FIRST TIME.
*/
/*
NOTES:
...
/* CONTRACT:
		IS FIXED SIZE:			CRXM__NOT(IS_TO_ENABLE_SIZE_OF_INTERNAL_BUFFER_FIELD)
		HAS CONSTRUCT:		 	1
		HAS DESTRUCT:		 	1
		IS COPYABLE:			IS_ELEMENT_COPYABLE
		HAS COPY CONSTRUCT:		1
		HAS MOVE CONSTRUCT:		CRXM__AND(CRXM__OR(DEFAULT_SIZE_OF_INTERNAL_BUFFER, 
												IS_TO_ENABLE_SIZE_OF_INTERNAL_BUFFER_FIELD),
										CRXM__NOT(IS_ELEMENT_RAW_MOVABLE))
		HAS MOVE DESTRUCT:		0
*/
...
#define CRX__C__Ring__DECLARE__13(pARRAY_TYPE_NAME, pARRAY_MEMBER_FUNCTIONS_PREFIX, pELEMENT_TYPE, \
		pSIZE_T, pSIZE_T_MAX, pDEFAULT_SIZE_OF_INTERNAL_BUFFER, pIS_TO_ENABLE_SIZE_OF_INTERNAL_BUFFER_FIELD, \
		pSTART_INDEX_DEFAULT_ALIGN_FACTOR, pIS_TO_ENABLE_ALIGN_FACTOR_FIELD, \
...
\
		...
		CRXM__OR(pIS_TO_ENABLE_ALIGN_FACTOR_FIELD, pSTART_INDEX_DEFAULT_ALIGN_FACTOR), \
		CRX__LIB__PUBLIC_C_FUNCTION(), CRX__LIB__PRIVATE_C_FUNCTION())
#define CRX__C__Ring__DEFINE__13(pARRAY_TYPE_NAME, pARRAY_MEMBER_FUNCTIONS_PREFIX, pELEMENT_TYPE, \
		pSIZE_T, pSIZE_T_MAX, pDEFAULT_SIZE_OF_INTERNAL_BUFFER, pIS_TO_ENABLE_SIZE_OF_INTERNAL_BUFFER_FIELD, \
		pSTART_INDEX_DEFAULT_ALIGN_FACTOR, pIS_TO_ENABLE_ALIGN_FACTOR_FIELD, \
		pFUNC_ELEMENT_DESTRUCTOR, pFUNC_ELEMENT_COPY_CONSTRUCTOR, \
		pFUNC_ELEMENT_MOVE_CONSTRUCTOR, pFUNC_ELEMENT_MOVE_DESTRUCTOR) \
...
#define _CRX__C__Ring__DEFINE(pARRAY_TYPE_NAME, pARRAY_MEMBER_FUNCTIONS_PREFIX, pELEMENT_TYPE, \
	...
	\
			...
			( \
				CRX__C__RING__ECHO__COPY_FROM_RING_RANGE(pFUNC_ELEMENT_COPY_CONSTRUCTOR, pELEMENT_TYPE, \
						tTarget, pThis->gPrivate_startIndex, pThis->gPrivate_capacity, 0, \
						pArray->gPrivate_elements, pArray->gPrivate_startIndex, pArray->gPrivate_capacity, \
						0, pArray->gPrivate_length) \
			) \
	\
			pThis->gPrivate_length = pArray->gPrivate_length; \
	...
	\
		...
		pARRAY_TYPE_NAME * vReturn = CRXM__IFELSE2(CRXM__NOT(pIS_TO_ENABLE_ALIGN_FACTOR_FIELD),  \
				...
				CRXM__IFELSE2(pIS_TO_ENABLE_SIZE_OF_INTERNAL_BUFFER_FIELD, \
				pARRAY_MEMBER_FUNCTIONS_PREFIX ## new(pWidth, 0, 0), \
				pARRAY_MEMBER_FUNCTIONS_PREFIX ## new(pWidth, 0))); \
	\
	...
	\
		...
		( \
			...
			{ \
				...
				{ \
					CRX_FOR(size_t tI = vStartIndex, tI < pThis->gPrivate_capacity, tI++) \
						{pFUNC_ELEMENT_DESTRUCTOR(pThis->gPrivate_elements + tI);} \
					CRX_ENDFOR \
	...
	PRIVATE void pARRAY_MEMBER_FUNCTIONS_PREFIX ## private_moveElements(pELEMENT_TYPE * pSourceBuffer, \
			size_t pStartIndex, size_t pLength, size_t pCapacity, \
			pELEMENT_TYPE * CRX_NOT_NULL pTargetBuffer) \
	{ \
	...
	\
					pARRAY_MEMBER_FUNCTIONS_PREFIX ## private_moveElements(pThis->gPrivate_buffer, pThis->gPrivate_startIndex, \
							...
							pThis->gPrivate_elements CRXM__IFELSE2(pHAS_ALIGN_FACTOR, + tStartIndex, )); \
	\
					pThis->gPrivate_startIndex = 0 CRXM__IFELSE2(pHAS_ALIGN_FACTOR, + tStartIndex, ); \
				)() \
	\
	...
	\
		...
		\
					CRX__C__Ring__DEFINE___SET_TO_ALIGN_FACTOR(pIS_TO_ENABLE_ALIGN_FACTOR_FIELD, \
							pSTART_INDEX_DEFAULT_ALIGN_FACTOR, pThis, tStartIndex) \
		\
					pARRAY_MEMBER_FUNCTIONS_PREFIX ## private_moveElements(pThis->gPrivate_elements, pThis->gPrivate_startIndex, \
							pThis->gPrivate_length, pThis->gPrivate_capacity, \
							tElements CRXM__IFELSE2(pHAS_ALIGN_FACTOR, + tStartIndex, )); \
		\
					free(pThis->gPrivate_elements); \
		...
		\
					...
					{ \
						memcpy(pThis->gPrivate_elements + pThis->gPrivate_startIndex + (pThis->gPrivate_capacity - pThis->gPrivate_startIndex), \
								pThis->gPrivate_elements, (tEndIndex + 1) * sizeof(pELEMENT_TYPE)); \
					} \
		\
					pThis->gPrivate_elements = tElements; \
		\
...
\
	...
	\
			vArray = pARRAY_MEMBER_FUNCTIONS_PREFIX ## private_new2(pArray, pStartIndex, tCapacity); \
		} \
	\
		if(vArray != NULL) \
		{ \
	...
	\
					...
					( \
						CRX__C__RING__ECHO__COPY_FROM_RING_RANGE(pFUNC_ELEMENT_COPY_CONSTRUCTOR, pELEMENT_TYPE, \
								pThis->gPrivate_elements, pThis->gPrivate_startIndex, pThis->gPrivate_capacity, 0, \
								vArray->gPrivate_elements, vArray->gPrivate_startIndex, vArray->gPrivate_capacity, \
								pStartIndex, pWidth) \
					) \
	\
	...
	\
		...
		( \
			...
			( \
				if(pThis->gPrivate_elements != NULL) \
					{pFUNC_ELEMENT_DESTRUCTOR(pThis->gPrivate_elements + (vIndexOfReturn & (pThis->gPrivate_capacity - 1)));} \
				else \
	...
	\
				...
				( \
					CRX__C__RING__ECHO__COPY_FROM_RING_RANGE(pFUNC_ELEMENT_COPY_CONSTRUCTOR, pELEMENT_TYPE, \
							pThis->gPrivate_elements, pThis->gPrivate_startIndex, pThis->gPrivate_capacity, pIndex, \
							vArray->gPrivate_elements, vArray->gPrivate_startIndex, vArray->gPrivate_capacity, \
							pStartIndex, pWidth) \
				) \
	\
	...
	\
		...
		( \
			CRX__C__RING__ECHO__COPY_FROM_RING_RANGE(pFUNC_ELEMENT_COPY_CONSTRUCTOR, pELEMENT_TYPE, \
					pThis->gPrivate_elements, pThis->gPrivate_startIndex, pThis->gPrivate_capacity, vIndexTransform.gNewIndex, \
					pArray->gPrivate_elements, pArray->gPrivate_startIndex, pArray->gPrivate_capacity, pStartIndex, \
					vIndexTransform.gNewCountToEnd) \
		) \
		CRX_SCOPE_END \
	}, ) \
	...
	PUBLIC bool pARRAY_MEMBER_FUNCTIONS_PREFIX ## insertElementCopiesAt2(pARRAY_TYPE_NAME * pThis, \
			size_t pIndex, pELEMENT_TYPE pElement, size_t pNumberOfCopies) \
	{ \
		return pARRAY_MEMBER_FUNCTIONS_PREFIX ## insertElementCopiesAt(pThis, pIndex, &pElement, \
				pNumberOfCopies); \
	}, ) \
	CRXM__IFELSE2(CRXM__AND(pIS_COPYABLE, CRXM__AND(CRXM__NOT(pFUNC_ELEMENT_COPY_CONSTRUCTOR), \
			CRXM__NOT(pFUNC_ELEMENT_DESTRUCTOR))), \
	...
	{ \
		pARRAY_MEMBER_FUNCTIONS_PREFIX ## fastInsertElementCopiesAt(pThis, pIndex, &pElement, \
				pNumberOfCopies); \
	}, ) \
	CRXM__IFELSE2(CRXM__AND(pIS_COPYABLE, CRXM__AND(CRXM__NOT(pFUNC_ELEMENT_COPY_CONSTRUCTOR), \
			CRXM__NOT(pFUNC_ELEMENT_DESTRUCTOR))), \
	PUBLIC void pARRAY_MEMBER_FUNCTIONS_PREFIX ## roundInsertElementCopiesAt2(pARRAY_TYPE_NAME * pThis, size_t pIndex, \
			pELEMENT_TYPE pElement, size_t pNumberOfCopies) \
	{ \
		pARRAY_MEMBER_FUNCTIONS_PREFIX ## roundInsertElementCopiesAt(pThis, pIndex, &pElement, \
				pNumberOfCopies); \
	}, ) \
	\
	...
	\
						...
						{ \
							...
							{tCapacity = pThis->gPrivate_capacity;} \
	\
						tElements = (pELEMENT_TYPE *)(malloc(tCapacity * sizeof(pELEMENT_TYPE))); \
	\
						if(tElements != NULL) \
						{ \
							pARRAY_MEMBER_FUNCTIONS_PREFIX ## private_moveElements(pThis->gPrivate_elements, \
									pThis->gPrivate_startIndex, pThis->gPrivate_length, pThis->gPrivate_capacity, \
									tElements + tStartIndex); \
	\
							pThis->gPrivate_startIndex = tStartIndex; \
							pThis->gPrivate_capacity = tCapacity; \
	\
	...
	\
					...
					( \
						pELEMENT_TYPE * tElements = (pELEMENT_TYPE *)(malloc(pThis->gPrivate_capacity * \
								sizeof(pELEMENT_TYPE))); \
	\
						if(tElements != NULL) \
						{ \
							pARRAY_MEMBER_FUNCTIONS_PREFIX ## private_moveElements(pThis->gPrivate_elements, \
									pThis->gPrivate_startIndex, pThis->gPrivate_length, pThis->gPrivate_capacity, tElements); \
	\
							pThis->gPrivate_startIndex = 0; \
	\
							free(pThis->gPrivate_elements); \
...
/* CONTRACT:
		...
		IS COPYABLE:			IS_ELEMENT_COPYABLE
		HAS COPY CONSTRUCT:		1
		HAS MOVE CONSTRUCT:		1
		HAS MOVE DESTRUCT:		0
		
	REMEMBER, THIS IS A GENERIC. THE ABOVE IS PER THE EXISTANCE OF FUNCTIONS AS REQUIRED IN THE
	STANDARD, BUT MAY NOT REFLECT THE ACTUAL CONTRACT OF THE GENERIC. HOWEVER, IT REMAINS THAT USER 
	CODE MUST FOLLOW THE ABOVE CONTRACT.
	
	THE TYPE IS GUARANTEED TO HAVE A FIXED SIZE IF THE SIZE OF THE INTERNAL BUFFER WAS SET TO ZERO,
	AND NO EXTERNAL BUFFER WAS PASSED TO IT. WHILE CLIENT CODE MUST CONTINUE TO FOLLOW THE CONTRACT
	AND THEREFORE NOT USE sizeof() DIRECTLY, THE GENERIC IS USABLE IN THIS CASE IN OTHER GENERICS 
	THAT EXPECT THE TYPE TO BE A FIXED SIZE.
*/
